...
const RegisterPage = () => {
  ...
  const { register: formRegister, handleSubmit, formState: { errors, touchedFields } } = useForm({
        ...
        <div className={styles.formWrap}>
          ...
          <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
            <div className={styles.inputWrap}>
              <input className={`${styles.input} ${errors.name ? styles.inputError : touchedFields.name ? styles.inputSuccess : ''}`} placeholder="Name" {...formRegister('name')} />
              {errors.name && <span className={styles.errorMsg}>{errors.name.message}</span>}
            </div>
            <div className={styles.inputWrap}>
              <input className={`${styles.input} ${errors.email ? styles.inputError : touchedFields.email ? styles.inputSuccess : ''}`} placeholder="Email" {...formRegister('email')} />
              {errors.email && <span className={styles.errorMsg}>{errors.email.message}</span>}
            </div>
            <div className={styles.inputWrap}>
              <div className={styles.passwordWrap}>
                <input className={`${styles.input} ${errors.password ? styles.inputError : touchedFields.password ? styles.inputSuccess : ''}`} type={showPassword ? 'text' : 'password'} placeholder="Password" {...formRegister('password')} />
