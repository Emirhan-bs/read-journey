...
import { fetchLibrary, addToLibrary, removeFromLibrary } from '../../redux/books/booksOperations';
import { selectLibrary, selectFilter, setFilter } from '../../redux/books/booksSlice';
        ...
        <div className={styles.addBook}>
          ...
          <form onSubmit={handleAddBook} className={styles.form}>
            <div className={styles.inputWrap}>
              ...
              <input name="title" className={styles.input} placeholder="Enter text" required />
            </div>
            <div className={styles.inputWrap}>
              <label className={styles.label}>The author:</label>
              <input name="author" className={styles.input} placeholder="Enter text" required />
            </div>
            <div className={styles.inputWrap}>
              <label className={styles.label}>Number of pages:</label>
              <input name="totalPages" type="number" min="1" className={styles.input} placeholder="0" required />
            </div>
      ...
      <section className={styles.content}>
        <div className={styles.contentHeader}>
          ...
          <div className={styles.selectWrap}>
            <button className={styles.selectBtn} onClick={() => setSelectOpen(p => !p)}>
              {filterOptions.find(o => o.value === filter)?.label}
              <Icon id="arrow" width={20} height={20} className={selectOpen ? styles.arrowUp : styles.arrowDown} />
            </button>
            {selectOpen && (
          ...
          <ul className={styles.bookList}>
            {filteredBooks.map(book => (
              <li key={book._id} className={styles.bookCard}>
                <div className={styles.bookImgWrap} onClick={() => setSelectedBook(book)}>
                  {book.imageUrl
                ...
                <button className={styles.removeBtn} onClick={() => handleRemove(book._id)}>
