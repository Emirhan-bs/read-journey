...
import { fetchRecommended, fetchLibrary, addToLibrary, removeFromLibrary } from './booksOperations';
...
const booksSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(fetchRecommended.pending, state => { state.isLoading = true; })
      .addCase(fetchRecommended.fulfilled, (state, action) => {
        state.isLoading = false;
        state.recommended = action.payload.results;
        state.totalPages = action.payload.totalPages;
      })
      .addCase(fetchRecommended.rejected, (state, action) => { state.isLoading = false; state.error = action.payload; })
      .addCase(fetchLibrary.fulfilled, (state, action) => { state.library = action.payload ?? []; })
      .addCase(addToLibrary.fulfilled, (state, action) => { state.library.push(action.payload); })
      .addCase(removeFromLibrary.fulfilled, (state, action) => {
        state.library = state.library.filter(book => book._id !== action.payload);
      });
  },
});
